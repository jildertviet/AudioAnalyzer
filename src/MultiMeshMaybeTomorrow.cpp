...
MultiMeshMaybeTomorrow::MultiMeshMaybeTomorrow(int size, ofFloatColor color){
    type = "MultiMeshMaybeTomorrow"; returnType();
    globalColor = color;
    numMeshes = size;
...
void MultiMeshMaybeTomorrow::deleteWithFade(int releaseTime, int attackTime){
    ...
    bFadeOut = true; fadeOutEnv->trigger = true; fadeOutEnv->setShape(attackTime,0.00001,release);
}
...
void MultiMeshMaybeTomorrow::moveHorizontal(){
    if(bMoveHorizontal){
        ...
        for(int i=0; i<numMeshes; i++){
            ...
            for(int j=0; j<meshes[i].getNumVertices(); j++){
                cout << j << endl;
                meshes[i].setVertex(j, meshes[i].getVertex(j)+ofVec2f(add*direction,0));
            }
        }
    }
